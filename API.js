...
// [신규] 1일 1회 데일리 뉴스 브리핑 (앵커 모드)
exports.getDailyBriefing = async function(req, res) {
    console.log("[getDailyBriefing] Request received");
    
    try {
        // 1. 캐싱 정책: "오늘 날짜" 파일이 있는지 확인
        const todayStr = moment().format('YYYY-MM-DD');
        const cacheDir = path.join(__dirname, 'audio_cache'); // 프로젝트 루트/audio_cache 폴더
        const cacheFilePath = path.join(cacheDir, `briefing_${todayStr}.mp3`);
        ...
        // 이미 오늘 생성된 브리핑이 있다면 바로 전송 (비용 0원)
        if (fs.existsSync(cacheFilePath)) {
            console.log(`[getDailyBriefing] Serving cached file: ${cacheFilePath}`);
        ...
        // --- 캐시가 없으면 새로 생성 시작 ---
        console.log("[getDailyBriefing] Generating new briefing...");

        // 2. 최신 뉴스 5개 가져오기
        const snapshot = await db.collection('eink-news')
            .orderBy('createdAt', 'desc')
            .limit(10)
            .get();

        if (snapshot.empty) {
            return res.status(404).send({ result: "fail", message: "뉴스가 충분하지 않습니다." });
        }
        ...
        // 3. LLM에게 앵커 대본 요청 (Gemini 사용 추천 - 빠르고 김)
        const scriptPrompt = `
            당신은 신뢰감 있는 'AI 뉴스 앵커'입니다. 아래 10가지 주요 뉴스를 바탕으로 3분 내외의 '오늘의 뉴스 브리핑' 대본을 작성해주세요.
            
            [뉴스 데이터]
            ${newsItems}

            [작성 가이드]
            1. 오프닝: "안녕하십니까, 오늘의 주요 뉴스 브리핑입니다."로 시작.
            2. 본문: 기사들을 자연스럽게 이어서 설명. (예: "다음 소식입니다.", "이어서 경제 분야입니다.")
            3. 톤앤매너: 하니다/습니다 체의 정중하고 명료한 뉴스 앵커 말투.
            4. 클로징: "이상으로 오늘의 브리핑을 마칩니다. 감사합니다."로 종료.
            5. 출력: 오직 읽을 대본 텍스트만 출력할 것 (마크다운, 특수문자 제외).
        `;

        // LLM 호출 (Gemini가 길고 정돈된 텍스트 생성에 유리하여 우선 사용)
        let anchorScript = "";
        try {
            anchorScript = await _callGemini(scriptPrompt);
        } catch (e) {
            console.warn("Gemini script gen failed, trying OpenAI...");
            anchorScript = await _callOpenAI(scriptPrompt);
        }

        // 4. Google Cloud TTS로 오디오 변환
        const ttsRequest = {
            input: { text: anchorScript },
            // ko-KR-Neural2-C: 굵직한 남성 앵커 톤 (가장 뉴스에 적합)
            voice: { languageCode: 'ko-KR', name: 'ko-KR-Neural2-C' },
        ...
        // 5. 파일 캐싱 (저장)
        fs.writeFileSync(cacheFilePath, audioContent, 'binary');
        console.log(`[getDailyBriefing] Saved new cache: ${cacheFilePath}`);
        ...
        logger.error("getDailyBriefing error: " + e.message);
        if (!res.headersSent) {
            res.status(500).send({ result: "fail", message: e.message });
        }
    }
